...
if [ $NUM_GPUS -eq 1 ]; then
    ...
    GRADIENT_ACCUMULATION_STEPS=4  # Effective batch size = 1 * 2 * 4 = 8
elif [ $NUM_GPUS -eq 4 ]; then
    GRADIENT_ACCUMULATION_STEPS=2  # Effective batch size = 1 * 4 * 2 = 8
else
    GRADIENT_ACCUMULATION_STEPS=$((8 / NUM_GPUS))  # Auto-adjust
